...
The metric chosen to evaluate solution quality is Mean Absolute Error (MAE), the average of the absolute differences between predicted values and true values.
...
```bash
git clone https://github.com/iuliia-kuznetsova/steel_temperature.git
cd steel_temperature
```
...
```bash
source .venv_steel/bin/activate # Linux/Mac
``` 
or
```bash
.\.venv_steel\Scripts\activate # Windows
```
...
### 3. Download the data (see **Data**) and place it into `./data`
...
For production deployment, more complex model architectures should be used to implement dynamic optimization instead of static temperature prediction. For example:
